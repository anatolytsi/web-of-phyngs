...
async function setPhyng(phyng: WoT.ConsumedThing, type: PhyngsType) {
    switch (type) {
        case "heaters":
            await phyng.writeProperty('temperature', JSON.stringify(350));
            break;
        case "acs":
            await phyng.writeProperty('temperature', JSON.stringify(283.15));
            await phyng.writeProperty('velocity', JSON.stringify(5));
            await phyng.invokeAction('turnOn');
            break;
        case "windows":
            await phyng.writeProperty('temperature', JSON.stringify(303.15));
            await phyng.writeProperty('velocity', JSON.stringify([-5, 0, 0]));
            await phyng.invokeAction('open');
            break;
        case "doors":
            await phyng.writeProperty('temperature', JSON.stringify(296.15));
            await phyng.writeProperty('velocity', JSON.stringify([5, 0, 0]));
            await phyng.invokeAction('open');
            break;
    }
